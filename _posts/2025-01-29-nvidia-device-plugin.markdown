...
1. device plugin 通过 unit socket 向 kubelet 注册自己，并上报节点可用扩展资源。
...
![java-javascript](/pics/deviceplugin-framework-overview.svg){:height="70%" width="70%"}
...
在该方法中，通过 DEVICE_LIST_STRATEGY（nvidia plugin 可配置参数）返回设备标志，主要有两种方式：env、mounts，对于 env 配置的环境变量为：`NVIDIA_VISIBLE_DEVICES=0`，这里的 `0` 是编号，还有一种策略是填 uuid，由环境变量 DEVICE_ID_STRATEGY 控制。对于 mounts 则表明容器需要挂载的 gpu 设备。nvidia 的主要实现在 getAllocateResponse 方法中，这里不再赘述。
...
在 containerd 环境下，可以通过下面命令查看容器所使用的 gpu 编号。plugin 将设备编号返回给 kubelet 之后，后者会将环境变量配置的到容器的运行时配置中，容器运行时在看到环境变量之后，会将对应 gpu 设备挂载到容器内部。这里的容器运行时是指 `nvidia-container-runtime`，它是 runc 的一个 warpper，负责拦截请求，并将设备挂载到容器内部。
```s
...
### 部署安装
#### 前置条件 nvidia-toolkit
nvidia plugin 不会在节点上安装 nvidia 驱动以及配置容器运行时，这个需要开发人员提前配置好。目前的约束条件有：
* NVIDIA drivers ~= 384.81
* nvidia-docker >= 2.0 || nvidia-container-toolkit >= 1.7.0 (>= 1.11.0 to use integrated GPUs on Tegra-based systems)
* nvidia-container-runtime configured as the default low-level runtime
* Kubernetes version >= 1.10

**配置容器运行时为 nvidia-contaienr-runtime**
...
2. 其次配置容器运行时为 nvidia-container-runtime
   ```s
   sudo nvidia-ctk runtime configure --runtime=docker
   ```
   该命令会修改 /etc/docker/daemon.json 并添加下面配置：
   ```json
   {
    "runtimes": {
        "nvidia": {
...
helm upgrade -i nvdp nvdp/nvidia-device-plugin \
  ...
  --version 0.17.0
```
另外 device plugin 支持安装 [GFD(gpu-feature-discovery)](https://github.com/NVIDIA/gpu-feature-discovery)，并且在安装 GFD 时，会默认安装 [NFD(Node Feature Discovery)](https://kubernetes-sigs.github.io/node-feature-discovery/stable/get-started/index.html)。后者主要生成节点相关特性相关 label 如：cpu 架构、指令集、内核版本，前者主要生成 gpu 相关 label，如：gpu 型号、卡数、驱动版本等。
```s
helm upgrade -i nvdp nvdp/nvidia-device-plugin \
  --version=0.17.0 \
  --namespace nvidia-device-plugin \
...
首先 mig 有三种策略：none,single,mixed，其中 none 表示禁用。single 表示一个 gpu 卡划分成统一中 profile，mixed 表示一个 gpu 卡可划分为多种 profile。gpu 卡类型与支持的 profile 可以参考 [8. Supported MIG Profiles](https://docs.nvidia.com/datacenter/tesla/mig-user-guide/#supported-mig-profiles)

这里以 nvidia-operator 为例说明如何配置 mig，具体分 single 和 mixed 方式。在介绍如何配置之前，我们先看 mig 相关的几个关键 node label:
...
我们通过修改 helm values.yaml 的方式，在安装或者更新 helm chart 的时候配置 mig 策略。在下面的配置中，配置了两个 mig 配置：一个是 `all-disabled`，另一个是 `custom-mig` 自定义配置，因为有两个 profile 1g.10gb 及 2g.20gb，因此是 mixed 策略。
```s
migManager:
  config:
